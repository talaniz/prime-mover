...
## Review-report clarification recovery — 2026-09-21

DOOM issue #18 published draft PR #19, but its first independent report included an empty check string alongside two actionable findings. Read-only replay of the actual report through Reviews.validate reproduced `Missing or invalid review check evidence`. No report was posted. Automatic recovery replayed the same completed result and exhausted its allowance (3); original review/cycle intents remain pending. The user requested fixing the failure and resuming work; no budget reset or state erasure is authorized.

Acceptance: preserve strict report validation and verbatim prior evidence, ask the same independent author to repair invalid evidence using the existing bounded clarification identities, never discard findings or publish invalid reports, retain target/authorization/deadline checks, and resume the existing job through the supported operator command with unchanged budgets/history. Persistent invalidity must stop after three total reports with invalid-review-report. No source implementation for DOOM is performed by the operator.

TDD red: build plus empty-check integration cases exited 1, both failing at Missing or invalid review check evidence. Green: code-review/reviews suites passed 28 tests. Added characterization assertions proving explicit resume preserves exhausted automatic-recovery count and original operation records. Original schema and persisted initial prompts remain compatible; no output-schema drift was introduced. Documentation explains operator recovery and prohibits duplicate continuation or deadline resets. Full checks and independent reviews follow.

Worker stopped before recovery to prevent repeated failed cycles; unrelated services untouched. Schema-2 SQLite backup `pre-review-clarification.sqlite` and isolated paused restore passed (3 jobs,52 operations,198 events). Original private artifacts and worktrees remain in place. Live resumption has not yet occurred.

Full `npm run check` passed: strict TypeScript and 294 tests (77 unit,208 integration,9 CLI E2E), zero failures. The explicit-resume preservation characterization passed within that suite. No additional refactor needed; independent reviews precede live deployment/resumption.

### Reviewed deployment and live continuation

Focused [code sign-off](https://github.com/talaniz/prime-mover/pull/6#issuecomment-5770293371) and subsequent [isolated E2E sign-off](https://github.com/talaniz/prime-mover/pull/6#issuecomment-5770318115) both reviewed `011f0c88edb11c48cc32e6907f091e9218bd48ef`. E2E exercised real Store/Reviews/CodeReviewRound/PrComments with fixture external adapters: legacy failed-result recovery, exhausted counter preservation, wrong proof rejection, same-author clarification, idempotent replay, persistent invalidity, revoked authorization and target drift. No live result was inferred from fixtures.

Deployed only the reviewed candidate worker release `primeMover-011f0c88edb1` after backup/restore and passing live doctor. Private rollback unit/baseline/receipt are in `/home/palpatine/.local/state/prime-mover/deployments/review-clarification/`. Runtime schema unchanged; no ledger replacement or unrelated service restart. Invoked supported `resume-code-review` for existing issue18 job `2123eecf-7030-404e-9e60-b9e1ee5470d9` under transient systemd `prime-mover-review-resume`, with resource limits and ExecStopPost returning control to the normal worker after exit. The supervisor stays stopped while the explicit continuation owns the lease.
