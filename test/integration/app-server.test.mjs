...
async function fixture(t, handler, timeoutMs = 5000) {
  const dir = await mkdtemp(join(tmpdir(), "pm-rpc-"));
  ...
  ws.on("connection", (peer) =>
    peer.on("message", (raw) => {
      ...
      if (m.method === "initialize")
        peer.send(
          JSON.stringify({ id: m.id, result: { userAgent: "codex/0.155.1" } }),
        );
      else handler(peer, m);
    }),
  );
  await new Promise((resolve) => server.listen(socket, resolve));
  const client = new AppServer(socket, timeoutMs);
  t.after(async () => {
    client.close();
    for (const p of ws.clients) p.terminate();
...
test("handshake precedes multiplexed lifecycle RPC and initialized notification", async (t) => {
  const { client, received } = await fixture(t, (p, m) => {
    if (m.id !== undefined)
      p.send(
        JSON.stringify({
          id: m.id,
...
test("server errors are actionable without reflecting secret-bearing remote text", async (t) => {
  const { client } = await fixture(t, (p, m) => {
    if (m.id !== undefined)
      p.send(
        JSON.stringify({
          id: m.id,
          error: { code: 401, message: "secret-value" },
        }),
      );
  });
  await client.connect();
  await assert.rejects(
    ...
    (e) => /401/.test(e.message) && !e.message.includes("secret-value"),
  );
});
test("timeout rejects ambiguous operation rather than retrying automatically", async (t) => {
  const { client, received } = await fixture(t, () => {}, 100);
  await client.connect();
  await assert.rejects(
    client.request("turn/start", {}),
    /timed out.*reconcile/i,
  );
  assert.equal(received.filter((m) => m.method === "turn/start").length, 1);
});
test("disconnect rejects pending RPC without issuing a replacement", async (t) => {
  const { client } = await fixture(t, (p, m) => {
    if (m.method === "turn/start") p.close();
  });
  await client.connect();
  await assert.rejects(
    client.request("turn/start", {}),
    /disconnect.*reconcile/i,
  );
});
    ...
    if (m.method === "thread/read") {
      p.send(
        JSON.stringify({
          ...
          params: { threadId: "owned", turnId: "turn" },
        }),
      );
      p.send(JSON.stringify({ id: m.id, result: { thread: { id: "owned" } } }));
    }
  });
  const events = [];
  client.on("notification", (e) => events.push(e));
  await client.connect();
  await client.request("thread/read", {});
  assert.equal(events[0]?.id, "approval");
  assert.equal(
    received.some((m) => m.id === "approval"),
    false,
  );
});
